...
# 3. Create an instance of GuidedBackprop
...
for i in range(10):  # Visualize attention maps for the first 10 subjects
    ...
    guided_gradients = guided_backprop.attribute(input_data, target=target_class)

    print(f"Predicted class for subject {i+1}: {subject_id}")

    # Convert guided gradients to numpy for visualization
    guided_gradients_np = guided_gradients.detach().numpy().squeeze()

    # Visualize the guided gradients overlayed with the input data
    plt.figure(figsize=(30, 15))

    plt.subplot(3, 2, 1)
    plt.imshow(guided_gradients_np[80, :, :], cmap='viridis')  # Superpone los gradientes (dimensión Z)
    plt.title('Sección central del volumen con gradientes (Z)')
    plt.axis('off')

    plt.subplot(3, 2, 3)
    plt.imshow(guided_gradients_np[:, 80, :], cmap='viridis')  # Superpone los gradientes (dimensión X)
    plt.title('Sección central del volumen con gradientes (X)')
    plt.axis('off')

    plt.subplot(3, 2, 5)
    plt.imshow(guided_gradients_np[:, :, 96], cmap='viridis')  # Superpone los gradientes (dimensión Y)
    plt.title('Sección central del volumen con gradientes (Y)')
    plt.axis('off')

    plt.savefig(f'guided_backprop_gradients_subject_{i+1}.png')
    plt.show()
