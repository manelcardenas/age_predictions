...
print('hola')
'''
# Función para obtener el uso de la memoria de la GPU
def get_gpu_memory_usage():
    try:
        result = subprocess.run(['nvidia-smi', '--query-gpu=memory.used', '--format=csv,nounits,noheader'], stdout=subprocess.PIPE)
        output = result.stdout.decode('utf-8').split('\n')
        memory_usage = [int(x) for x in output if x]
        return memory_usage
    except Exception as e:
        print("Error al obtener el uso de la memoria de la GPU:", e)
        return None
'''
...
keys_train_val, keys_test, age_dist_train_val, age_dist_test = train_test_split(keys, age_dist_array, test_size=0.2, random_state=42) # (train/val) / test      stratify=age_dist
keys_train, keys_val, age_dist_train, age_dist_val = train_test_split(keys_train_val, age_dist_train_val, test_size=0.125, random_state=42)#,  train/val  stratify=age_dist_train
...
val_loader = DataLoader(dataset_val, batch_size=8, sampler=val_sampler, num_workers=10, pin_memory=True) #DROP_LAST 
test_loader = DataLoader(dataset_test, batch_size=8, sampler=test_sampler, num_workers=10, pin_memory=True) #DROP_LAST 
'''
'''
#dataloader
...
if dist.is_primary() and False:
    print(f'main GPU {dist.is_primary()} starting wandb')
    wandb.init(project='Brain_age', entity='manelcardenas')
...
def main():
    ...
    if torch.cuda.device_count() > 1 and device.type == 'cuda':
        ...
        model = DDP(model, device_ids=[dist.get_local_rank()])
    ...
    for epoch in range(num_epochs):
        ...
        running_loss = 0.0
        running_mae = 0.0
        # Obtener uso de la memoria de la GPU
        #gpu_memory_usage = get_gpu_memory_usage()

        for j, (inputs, age_dist, age_real, subject_ids) in enumerate(train_loader):
            optimizer.zero_grad()
            ...
            if gpu_memory_usage is not None:
                print(f'GPU {j % torch.cuda.device_count()}: {gpu_memory_usage[j % len(gpu_memory_usage)]} MiB')
                for j, memory in enumerate(gpu_memory_usage):
                    wandb.log({'gpu_memory_usage': memory}, step=epoch * len(train_loader) + j)
            '''
...
if __name__ == "__main__":
    dist.launch(main, n_gpu_per_machine=3, n_machine=1, machine_rank=0, dist_url="auto")

'''
if __name__ == "__main__":
    torch.distributed.launcher.run()
'''
